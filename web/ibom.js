...
function syncStartupOverlayTheme() {
  ...
  overlay.classList.remove("theme-light", "theme-dark", "theme-blue", "theme-green", "theme-apple", "dark");
  ...
  if (root.classList.contains("theme-blue")) {
    overlay.classList.add("theme-blue");
  }
  if (root.classList.contains("theme-green")) {
    overlay.classList.add("theme-green");
  }
  if (root.classList.contains("theme-apple")) {
    overlay.classList.add("theme-apple");
  }
...
function initStartupOverlay() {
  ...
  if (title && pcbdata && pcbdata.metadata && pcbdata.metadata.title) {
    title.textContent = pcbdata.metadata.title;
  }
  if (subtitle && pcbdata && pcbdata.metadata) {
    var meta = [];
    if (pcbdata.metadata.company) {
      meta.push(pcbdata.metadata.company);
    }
...
function populateBomHeader() {
  ...
  if (settings.bommode == "netlist") {
    th = createColumnHeader(t("net_name"), "bom-netname", (a, b) => {
      if (a > b) return -1;
      if (a < b) return 1;
      return 0;
    });
    ...
    tr.appendChild(createColumnHeader(t("references_col"), "References", (a, b) => {
      var i = 0;
    ...
    tr.appendChild(createColumnHeader(t("value_col"), "Value", (a, b) => {
      return valueCompare(a[5], b[5], a[1], b[1]);
    }));
    tr.appendChild(createColumnHeader(t("footprint_col"), "Footprint", (a, b) => {
      if (a[2] != b[2]) return a[2] > b[2] ? 1 : -1;
      else return 0;
    }));
    if (settings.bommode == "grouped") {
      tr.appendChild(createColumnHeader(t("quantity_col"), "Quantity", (a, b) => {
        return a[3].length - b[3].length;
      }));
    }
  }
