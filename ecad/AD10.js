...
var IBOM_SCRIPT_MESSAGES = {
    "open_pcb": {
        "en": "Please open a PCB document",
        "zh": "\u8bf7\u5148\u6253\u5f00 PCB \u6587\u6863",
        "ja": "PCB \u30c9\u30ad\u30e5\u30e1\u30f3\u30c8\u3092\u958b\u3044\u3066\u304f\u3060\u3055\u3044",
        "ko": "PCB \ubb38\uc11c\ub97c \uba3c\uc800 \uc5f4\uc5b4 \uc8fc\uc138\uc694",
        "de": "Bitte offnen Sie ein PCB-Dokument",
        "fr": "Veuillez ouvrir un document PCB",
        "ru": "\u0421\u043d\u0430\u0447\u0430\u043b\u0430 \u043e\u0442\u043a\u0440\u043e\u0439\u0442\u0435 \u0434\u043e\u043a\u0443\u043c\u0435\u043d\u0442 PCB"
    },
    "current_not_pcb": {
        "en": "Error: Current document is not a PCB document. Please switch to the target PcbDoc first.",
        "zh": "\u9519\u8bef\uff1a\u5f53\u524d\u6d3b\u52a8\u6587\u6863\u4e0d\u662f PCB\uff0c\u8bf7\u5148\u5207\u6362\u5230\u8981\u5bfc\u51fa\u7684 PcbDoc",
        "ja": "\u30a8\u30e9\u30fc: \u73fe\u5728\u306e\u30a2\u30af\u30c6\u30a3\u30d6\u306a\u30c9\u30ad\u30e5\u30e1\u30f3\u30c8\u306f PCB \u3067\u306f\u3042\u308a\u307e\u305b\u3093\u3002\u5bfe\u8c61\u306e PcbDoc \u306b\u5207\u308a\u66ff\u3048\u3066\u304f\u3060\u3055\u3044",
        "ko": "\uc624\ub958: \ud604\uc7ac \ud65c\uc131 \ubb38\uc11c\ub294 PCB\uac00 \uc544\ub2d9\ub2c8\ub2e4. \ub0b4\ubcf4\ub0bc PcbDoc\uc73c\ub85c \uba3c\uc800 \uc804\ud658\ud574 \uc8fc\uc138\uc694",
        "de": "Fehler: Das aktuelle Dokument ist kein PCB-Dokument. Bitte wechseln Sie zuerst zum zu exportierenden PcbDoc.",
        "fr": "Erreur : le document actif n'est pas un document PCB. Basculez d'abord vers le PcbDoc a exporter.",
        "ru": "\u041e\u0448\u0438\u0431\u043a\u0430: \u0442\u0435\u043a\u0443\u0449\u0438\u0439 \u0430\u043a\u0442\u0438\u0432\u043d\u044b\u0439 \u0434\u043e\u043a\u0443\u043c\u0435\u043d\u0442 \u043d\u0435 \u044f\u0432\u043b\u044f\u0435\u0442\u0441\u044f PCB. \u0421\u043d\u0430\u0447\u0430\u043b\u0430 \u043f\u0435\u0440\u0435\u043a\u043b\u044e\u0447\u0438\u0442\u0435\u0441\u044c \u043d\u0430 \u044d\u043a\u0441\u043f\u043e\u0440\u0442\u0438\u0440\u0443\u0435\u043c\u044b\u0439 PcbDoc."
    }
};

function getAdUiLang() {
    var lang = "en";
    try {
        var shell = new ActiveXObject("WScript.Shell");
        var localeName = "";
        var prefFound = false;
        var useLocalizedResources = false;
        var useLocalizedDialogs = false;

        try {
            localeName = String(shell.RegRead("HKCU\\Control Panel\\International\\LocaleName") || "").toLowerCase();
        } catch (e) {
            localeName = "";
        }
        ...
        var appData = "";
        try {
            appData = String(shell.ExpandEnvironmentStrings("%APPDATA%") || "");
        } catch (e) {
            appData = "";
        }
        ...
        if (appData !== "") {
            ...
            var altiumRoot = appData + "\\Altium";
            if (fso.FolderExists(altiumRoot)) {
                var folder = fso.GetFolder(altiumRoot);
                ...
                for (; !subfolders.atEnd(); subfolders.moveNext()) {
                    ...
                    var subName = String(subfolder.Name || "");
                    if (subName.indexOf("Altium Designer ") !== 0) {
                        continue;
                    }
                    var iniPath = subfolder.Path + "\\ExportPreferences\\WorkspaceManager_General.ini";
                    if (!fso.FileExists(iniPath)) {
                        continue;
                    }
                    var stamp = new Date(fso.GetFile(iniPath).DateLastModified).getTime();
                ...
                if (latestIni !== "") {
                    prefFound = true;
                    var iniFile = TIniFile.Create(latestIni);
                    useLocalizedResources = iniFile.ReadBool("SysPref_General", "Use Localized Resources", false);
                    useLocalizedDialogs = iniFile.ReadBool("SysPref_General", "Use Localized Dialogs", false);
                    iniFile.Free;
                }
            }
        }

        if (!prefFound || !useLocalizedResources || !useLocalizedDialogs) {
            return "en";
        }
...
function ibomMsg(key) {
    var lang = getAdUiLang();
    if (IBOM_SCRIPT_MESSAGES[key] && IBOM_SCRIPT_MESSAGES[key][lang]) {
        return IBOM_SCRIPT_MESSAGES[key][lang];
    }
    if (IBOM_SCRIPT_MESSAGES[key] && IBOM_SCRIPT_MESSAGES[key]["en"]) {
        return IBOM_SCRIPT_MESSAGES[key]["en"];
    }
    return key;
}
...
function parsePcb(config) {
    ...
    // parse_board
    if (PCBServer == null) {
        showmessage(ibomMsg("open_pcb"));
        return false;
    }
    PCBServer.PreProcess;
    var board = PCBServer.GetCurrentPCBBoard();
    if (board == null) {
        showmessage(ibomMsg("current_not_pcb"));
        return false;
    }
    pcb["board"] = board;
